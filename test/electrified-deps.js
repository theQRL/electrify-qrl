describe('[electrify] .electrified dependencies', function(){
  ...
  var npm_dir  = path.join(root_dir, '.npm', 'node_modules', 'electrify-qrl');
  ...
  before(function(done){
    ...
    // crates a sample app and add the packagehttps://github.com/meteor/leaderboard
    spawn(git_bin, ['clone', '--depth=1', ''], {
      cwd: tests_dir,
      stdio: stdio_config
    }).on('exit', function(){
  ...
  it('should ensure .electrify dependencies', function(done) {
    ...
    var node_mods_electrify = path.join(node_mods, 'electrify-qrl');
    ...
    pkg.dependencies['electrify-qrl'] = '0.0.1';
